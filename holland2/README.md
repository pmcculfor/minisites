...
- **Weather:** National Weather Service point forecast for **Holland, MI** (Grand Rapids office, grid from `42.7875, -86.1089`). Day tiles show that official high/low. Open-Meteo at the same point is only used for **past** trip days NWS no longer publishes — not for today or upcoming days (its inland-model highs can run ~10° hotter than the lakeshore forecast). Browser requests omit custom NWS headers so weather.gov CORS succeeds. °F and mph, Eastern Time.
